...
class FormatParser
{
     ...
     * @param string $value The string to parse. Must be a valid attribute or method defined in the Generator loaded
     *
     * @return Format The format object to use when invoking the generator
     * @throws \Exception If the value provided does not match a valid property or method based on the generator loaded
     */
    public function parse($value)
    {
        ...
        if (empty($args)) {
            ...
            if (count($args) > count($generatorMethod->getParameters())) {
                throw new \Exception("Invalid format provided: $value. ".
                    "Reason: Too many parameters for Faker method: $name. ".
                    "Expected: " . count($generatorMethod->getParameters()) . ', Actual: ' . count($args));
            }
        }
