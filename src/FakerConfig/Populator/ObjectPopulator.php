    ...
    public function populate($object)
    {
        ...
        if (!isset($classes)) {
            throw new \InvalidArgumentException(
                'Failed to retrieve class hierarchy. Make sure you passed a valid object'
            );
        }
        ...
        foreach ($properties as $property) {
            ...
            foreach ($classes as $class) {
                ...
                } elseif ($class == $property->getDeclaringClass()->getName()) {
                    // We have reached the class where the property has been declared, check for wildcard as well
                    $format = $this->configGuesser->guessFormat($class, $property->getName());
                    break;
                }
